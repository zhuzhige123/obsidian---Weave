<script lang="ts">
  ...
  // @deprecated 摘录列表视图已弃用
  // import ExtractListView from '../incremental-reading/ExtractListView.svelte';
  //  导入国际化
  ...
  let internalFilter = $state<DeckFilter>((() => {
    try {
      const saved = localStorage.getItem('weave-deck-mode-filter') as DeckFilter;
  ...
  function handleFilterSelect(filter: DeckFilter) {
    ...
    if (externalOnFilterSelect) {
      ...
      localStorage.setItem('weave-deck-mode-filter', filter);
    }
