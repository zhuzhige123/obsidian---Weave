<script lang="ts">
  ...
  let selectedFilter = $state<'memory' | 'question-bank' | 'incremental-reading' | 'parent' | 'child' | 'all'>((() => {
    try {
      const saved = localStorage.getItem('weave-deck-mode-filter');
  ...
  onMount(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const savedView = savedData?.deckView || localStorage.getItem('weave-deck-view'); // 兼容旧版localStorage
        ...
        if (localStorage.getItem('weave-deck-view') && savedData?.deckView !== localStorage.getItem('weave-deck-view')) {
          const migratedView = localStorage.getItem('weave-deck-view');
        ...
        // 降级到localStorage（兼容）
        const savedView = localStorage.getItem('weave-deck-view');
        if (savedView && ['kanban', 'grid'].includes(savedView)) {
          currentView = savedView as typeof currentView;
          window.dispatchEvent(new CustomEvent('Weave:deck-view-change', { detail: currentView }));
        ...
        item
          ...
          .onClick(async () => {
            ...
            try {
              ...
              // 降级到localStorage（兼容）
              localStorage.setItem('weave-deck-view', view.id);
            }
  ...
  function handleFilterSelect(filter: 'memory' | 'reading' | 'question-bank' | 'incremental-reading' | 'parent' | 'child' | 'all') {
    ...
    localStorage.setItem('weave-deck-mode-filter', selectedFilter);
  ...
  async function loadDeckTree() {
    try {
      ...
      const hasStoredState = localStorage.getItem('weave-deck-expanded-state');
  ...
  function saveExpandedState() {
    try {
      ...
      localStorage.setItem('weave-deck-expanded-state', JSON.stringify(stateArray));
    } catch (error) {
  ...
  function loadExpandedState() {
    try {
      const saved = localStorage.getItem('weave-deck-expanded-state');
      if (saved) {
