<script lang="ts">
  ...
  function handleVisibilityChange(key: keyof typeof columnVisibility, value: boolean) {
    ...
    // 持久化到 localStorage
    try {
      localStorage.setItem('weave-column-visibility', JSON.stringify(columnVisibility));
    } catch (error) {
  ...
  function handleOrderChange(newOrder: ColumnOrder) {
    ...
    // 持久化到 localStorage
    try {
      localStorage.setItem('weave-column-order', JSON.stringify(newOrder));
    } catch (error) {
  ...
  onMount(() => {
    ...
    // 加载列可见性设置
    const savedColumnVisibility = localStorage.getItem('weave-column-visibility');
    ...
    // 加载列顺序设置
    const savedColumnOrder = localStorage.getItem('weave-column-order');
    if (savedColumnOrder) {
      try {
              ...
              try {
                if (source.indexFilePath) {
                  ...
                  if (indexFile instanceof TFile) {
                    await plugin.app.vault.delete(indexFile);
                    // 记录索引文件的父文件夹
