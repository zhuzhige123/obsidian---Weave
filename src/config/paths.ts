 ...
 * @returns 备份文件夹路径
 */
export function getPluginBackupPath(pluginId: string = 'weave'): string {
  return `.obsidian/plugins/${pluginId}/backups`;
}
...
export function getIndexPath(indexName: string): string {
  return `${PLUGIN_PATHS.indices.root}/${indexName}.json`;
}
