...
export class DataManagementService {
  ...
  private async importMigrationConflicts(v2Paths: ReturnType<typeof getV2Paths>): Promise<{ importedCards: number; importedDecks: number; errors: string[] }> {
    ...
    const absDirPath = nodePath.join(basePath, conflictDir);

    if (!nodeFs.existsSync(absDirPath)) {
      return result;
    }

    let allFileNames: string[];
    try {
      allFileNames = nodeFs.readdirSync(absDirPath);
    } catch (e) {
    ...
    const readConflictFile = (fileName: string): string | null => {
      try {
        return nodeFs.readFileSync(nodePath.join(absDirPath, fileName), 'utf-8');
      } catch (e) {
    ...
    for (const deckFileName of deckFileNames) {
      try {
        const raw = readConflictFile(deckFileName);
        if (!raw) continue;
    ...
    for (const cardFileName of cardFileNames) {
      try {
        const raw = readConflictFile(cardFileName);
        if (!raw) continue;
        const parsed = JSON.parse(raw);
    ...
    if (result.importedCards > 0 || result.importedDecks > 0) {
      ...
      for (const f of allFileNames) {
        try {
          nodeFs.unlinkSync(nodePath.join(absDirPath, f));
        } catch { }
      }
      try {
        const remaining = nodeFs.readdirSync(absDirPath);
        if (remaining.length === 0) {
          nodeFs.rmdirSync(absDirPath);
        }
      } catch { }
    }
  ...
  private async cleanupEmptyCardFiles(v2Paths: ReturnType<typeof getV2Paths>): Promise<{ deleted: number; merged: number; errors: string[] }> {
    ...
    try {
      ...
      const absCardsDir = nodePath.join(basePath, cardsDir);

      if (!nodeFs.existsSync(absCardsDir)) return result;

      const files = nodeFs.readdirSync(absCardsDir).filter((f: string) => f.endsWith('.json'));
      ...
      for (const f of files) {
        ...
        const absPath = nodePath.join(absCardsDir, f);
        let cards: Card[] = [];
        try {
          const raw = nodeFs.readFileSync(absPath, 'utf-8');
          const parsed = JSON.parse(raw);
      ...
      for (const f of toDelete) {
        try {
          nodeFs.unlinkSync(nodePath.join(absCardsDir, f));
          result.deleted++;
        } catch (e) {
  ...
  private async renameDotManifestFiles(v2Paths: ReturnType<typeof getV2Paths>): Promise<number> {
    ...
    try {
      ...
      const nodeFs = require('fs') as typeof import('fs');
      const nodePath = require('path') as typeof import('path');
      const mediaDir = nodePath.join(basePath, v2Paths.memory.root, 'media');

      if (!nodeFs.existsSync(mediaDir)) return 0;

      const subdirs = nodeFs.readdirSync(mediaDir);
      for (const sub of subdirs) {
        const subPath = nodePath.join(mediaDir, sub);
        if (!nodeFs.statSync(subPath).isDirectory()) continue;
        const dotManifest = nodePath.join(subPath, '.manifest.json');
        const newManifest = nodePath.join(subPath, 'manifest.json');
        if (nodeFs.existsSync(dotManifest)) {
          if (!nodeFs.existsSync(newManifest)) {
            nodeFs.renameSync(dotManifest, newManifest);
          } else {
            nodeFs.unlinkSync(dotManifest);
          }
          renamed++;
        }
      }
