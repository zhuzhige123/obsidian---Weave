...
export class IRDeckManager {
    ...
    collectReadingMaterialsForDeck(materials, deckIdentifiers) {
    ...
    async cleanupReadingArtifactsForDeck(deckId, deckName) {
        const deckIdentifiers = new Set([deckId, deckName]
            ...
            if (!material.parentMaterialId && material.filePath?.toLowerCase().endsWith('.md')) {
                ...
                if (file instanceof TFile) {
                    try {
                        ...
                        logger.warn(`[IRDeckManager] 清理 Markdown 增量阅读 YAML 失败: ${material.filePath}`, error);
                    }
                }
            }
            try {
