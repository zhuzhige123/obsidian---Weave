...
export class IRPdfBookmarkTaskService {
    ...
    async deleteTasksByDeckIdentifiers(deckIds) {
        ...
        return this.deleteTasksByPredicate(task => identifiers.has(String(task?.deckId || '').trim()), '[IRPdfBookmarkTaskService] 已按牌组标识删除任务:', { deckIds: Array.from(identifiers) });
    }
    async deleteTasksByPdfPaths(pdfPaths) {
        const paths = this.toNormalizedSet(pdfPaths);
        return this.deleteTasksByPredicate(task => paths.has(String(task?.pdfPath || '').trim()), '[IRPdfBookmarkTaskService] 已按 PDF 路径删除任务:', { pdfPaths: Array.from(paths) });
    }
