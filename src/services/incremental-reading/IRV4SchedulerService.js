...
export class IRV4SchedulerService {
    ...
    async getStudyQueueV4(deckPath, options) {
        ...
        try {
            ...
            logger.warn('[IRV4SchedulerService] 读取 EPUB 书签任务失败（将继续仅使用 chunk 队列）:', error);
        }
        const allBlocks = [...chunkBlocks, ...pdfTaskBlocks, ...epubTaskBlocks];
    ...
    async completeBlockV4(blockV4, data, deckPath = '') {
        ...
        if (isPdfBookmarkTaskId(updatedBlock.id)) {
            ...
            await this._epubBookmarkTaskService.recordTaskInteraction(updatedBlock.id, data.readingTimeSeconds, {
                extracts: data.createdExtractCount,
                cardsCreated: data.createdCardCount,
                notesWritten: data.createdNoteCount
            });
        ...
        if (!isPdfBookmarkTaskId(updatedBlock.id) && !isEpubBookmarkTaskId(updatedBlock.id) && advancedSettings.enableTagGroupPrior) {
            try {
