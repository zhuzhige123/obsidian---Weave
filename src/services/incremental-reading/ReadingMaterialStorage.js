...
import { getV2PathsFromApp, PLUGIN_PATHS } from '../../config/paths';
/**
 * V2.0 存储路径配置
 * 材料数据合并到 weave/incremental-reading/materials/
 * 锚点缓存移到插件目录 .obsidian/plugins/weave/cache/
 */
const ANCHORS_CACHE_PATH = PLUGIN_PATHS.cache.anchors;
/**
 * 阅读材料存储服务
 */
export class ReadingMaterialStorage {
    ...
    get storagePaths() {
        ...
        return {
            ...
            ANCHORS_CACHE: ANCHORS_CACHE_PATH,
     ...
     * 通过UUID获取阅读材料
     */
    async getMaterialById(uuid) {
        return this.materialsCache.get(uuid) || null;
    }
    /**
     * 通过文件路径获取阅读材料
     */
    async getMaterialByPath(filePath) {
