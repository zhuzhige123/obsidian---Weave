...
export class ProgressiveClozeGateway {
     ...
     */
    async processContentChange(oldCard, newContent, dataStorage, onConfirmNeeded) {
     ...
     * 需要用户确认后才会删除所有子卡片
     */
    async handleConvertToSimple(oldCard, newContent, dataStorage, onConfirmNeeded) {
        ...
        if (childCards.length > 0 && onConfirmNeeded) {
            const hasStudiedChildren = childCards.some(c => isProgressiveClozeChild(c) && c.fsrs && c.fsrs.reps > 0);
